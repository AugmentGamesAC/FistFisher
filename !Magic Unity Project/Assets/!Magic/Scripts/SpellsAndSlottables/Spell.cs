...
public class Spell : ICastingInterface
{
    ...
    void ApplyRotation(SpellDescription.Aiming aiming)
    {
        ...
        m_targeting.transform.Rotate(
            aiming.HasFlag(SpellDescription.Aiming.ReverseX) ? -90 : 0,
            aiming.HasFlag(SpellDescription.Aiming.ReverseY) ? -90 : 0,
            aiming.HasFlag(SpellDescription.Aiming.ReverseZ) ? -90 : 0);
    ...
    Vector3 ApplyTransfrom(SpellDescription.Aiming aiming)
    {
        ...
        Vector3 dimentions = m_targeting.GetComponentInChildren<Renderer>().bounds.size;
        return new Vector3(
    ...
    void ResolveAimingFromFingerEnd()
    {
        ...
        if (!Physics.Raycast(m_AimPoint.StartingPoint, m_AimPoint.Direction, out hit, m_MaxDistance))
        {
            Vector3 pos = m_AimPoint.StartingPoint + m_AimPoint.Direction.normalized * m_MaxDistance;
            m_targeting.transform.SetPositionAndRotation(m_AimPoint.Direction * m_MaxDistance, Quaternion.FromToRotation(m_targeting.transform.up, hit.normal) * m_targeting.transform.rotation);
        }
        if (m_showRayTrace)
        ...
        ApplyRotation(m_Start.m_aiming);
        m_targeting.transform.position += ApplyTransfrom(m_Start.m_aiming);
    }
    ...
    public bool Interact(InteractionType interaction)
    {
        ...
        if ((interaction.HasFlag(InteractionType.BypassAiming)) || (interaction.HasFlag(InteractionType.StartCasting)) && (m_isAiming))
        {
            m_spellOwner.UseMana(m_manaCostPerSecond);
            m_targeting.GetComponent<SpellInstance>().m_IsAiming = false;
            m_isAiming = false;
            if(!_Start.m_aiming.HasFlag(SpellDescription.Aiming.CenteredBoxToFingerTip))
                    m_targeting.gameObject.transform.SetParent(null);
            if (_Start.m_effect == SpellDescription.Effect.Summon)
                m_targeting.gameObject.layer = 0;
        }
