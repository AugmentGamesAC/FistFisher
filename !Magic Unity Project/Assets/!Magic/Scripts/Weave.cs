...
public class Weave : GestureHandle {
	...
	public List<int> m_AllGestureIDs;
	...
	void HandleOnPlayerGestureAdd(long gestureId, Dictionary<int, int> result)
	{
		...
		if (count >= SAMPLES)
		{
			airsigManager.SetPlayerGesture(new List<int>() { currentPlayerGestureTarget}, false);
	...
	void Awake()
	{
        m_AllGestureIDs = new List<int>() { 101, 102, 103 };//, 102, 103 };
	...
	void LateUpdate()
	{
        ...
        GauntletBase Gauntlet = airsigManager.m_Player.GetComponentInChildren<GauntletBase>();
        if(Gauntlet!=null)
        {
            Gauntlet.AimSpell(spellID);
            print(string.Format("Passing spell id {0}", spellID));
        }
	...
	public void StartWeaving(Valve.VR.InteractionSystem.Hand hand)
	{
        ...
        GauntletBase gb = player.GetComponentInChildren<GauntletBase>();

        if(gb.m_QueuedSpell != null && gb.m_QueuedSpell.m_isAiming)
        {
            gb.CastSpell();
            return;
        }
